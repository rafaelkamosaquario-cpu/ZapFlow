...
export const FERRAMENTAS_DEFINICOES = [
  ...
  },
  {
    ...
    description: "Panorama geral do negócio no período: funil comercial (quantos clientes em cada etapa), follow-ups pendentes e métricas de campanhas (enviadas/respostas/taxa). Use pra perguntas gerais tipo 'como está o negócio' ou 'me dê um resumo'. NÃO traz nomes de clientes específicos nem motivos -- pra isso use buscar_radar_comercial.",
    parameters: {
      type: "object",
      properties: {
        periodo: { type: "string", enum: ["hoje", "7d", "30d", "mes"], description: "Período a considerar. Padrão: hoje." },
  ...
  {
    type: "function",
    name: "buscar_radar_comercial",
    description: "Lista, cliente por cliente e com o motivo exato, quem precisa de atenção agora: quem respondeu e não recebeu retorno, próxima ação atrasada/hoje, negociação parada, proposta sem próximo passo, cliente sem responsável. Use pra perguntas tipo 'quem precisa da minha atenção', 'quais clientes estão parados', 'quem está esperando resposta', 'quais negociações precisam de retorno'. Pode ser filtrado por vendedor.",
    parameters: {
      type: "object",
      properties: {
        vendedorNome: { type: "string", description: "Nome (ou parte do nome) de um vendedor específico, pra ver só a carteira dele. Deixe vazio pra ver de todos." },
      },
      required: ["vendedorNome"],
      additionalProperties: false,
    },
 ...
 */
export function criarExecutores(tenant, { agregarDesempenhoEquipe, agregarClienteDetalhe, phoneKey, montarRadar } = {}) {
  return {
    ...
    async buscar_desempenho_equipe({ periodo }) {
      ...
      return {
        ...
        porVendedor: r.vendedores.map((v) => ({
          ...
          clientesResponsaveis: v.clientesResponsaveis, followupsAtrasados: v.followupsAtrasados,
        })),
      };
    },
    ...
    /** Reaproveita 100% a lógica do Radar Comercial (mesma tela "Precisa da sua atenção") -- não recalcula nada. */
    async buscar_radar_comercial({ vendedorNome } = {}) {
      if (!montarRadar) return { erro: "Indisponível." };
      let vendedorId = "";
      const vendedores = await repo.usuariosRepo.listVendedores(tenant.empresa.id);
      if (vendedorNome) {
        const termo = vendedorNome.toLowerCase();
        const alvo = vendedores.find((v) => (v.name || v.username || "").toLowerCase().includes(termo));
        if (!alvo) return { erro: `Nenhum vendedor encontrado com o nome "${vendedorNome}".` };
        vendedorId = alvo.id;
      }
      const todasVisitas = await repo.visitasRepo.listForEmpresa(tenant.empresa.id);
      const radar = montarRadar(tenant, vendedores, todasVisitas, vendedorId);
      return {
        resumo: radar.resumo, total: radar.total,
        itens: radar.itens.slice(0, 30).map((i) => ({ prioridade: i.prioridade, cliente: i.client.name, telefone: i.client.phone, situacao: i.texto })),
      };
    },
  };
}
